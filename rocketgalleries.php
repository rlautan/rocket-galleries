...
class RocketGalleries {
    ...
    public function register_all_styles() {
        ...
        wp_register_style( 'jquery-booklet', plugins_url( dirname( plugin_basename( self::get_file() ) ) . DIRECTORY_SEPARATOR . 'lib' . DIRECTORY_SEPARATOR . 'jquery.booklet' . DIRECTORY_SEPARATOR .'css'. DIRECTORY_SEPARATOR .'jquery.booklet'. '.css' ), false, self::$version );
    ...
    public function register_all_scripts() {
        ...
        wp_register_script( 'jquery-mobile', "http" . ($_SERVER['SERVER_PORT'] == 443 ? "s" : "") . "://ajax.googleapis.com/ajax/libs/jquerymobile/1.4.3/jquery.mobile.min.js", array('jquery'), null );
        ...
        wp_register_script( 'jquery-easing',  plugins_url( dirname( plugin_basename( self::get_file() ) ) . DIRECTORY_SEPARATOR . 'lib' . DIRECTORY_SEPARATOR . 'jquery.booklet' . DIRECTORY_SEPARATOR . 'js' . DIRECTORY_SEPARATOR . 'jquery.easing' . $ext ), array( 'jquery' ), self::$version, true );
        wp_register_script( 'jquery-booklet',  plugins_url( dirname( plugin_basename( self::get_file() ) ) . DIRECTORY_SEPARATOR . 'lib' . DIRECTORY_SEPARATOR . 'jquery.booklet' . DIRECTORY_SEPARATOR . 'js' . DIRECTORY_SEPARATOR . 'jquery.booklet' . $ext ), array( 'jquery-easing' ), self::$version, true );
    ...
    public function enqueue_gallery_assets() {
        ...
        add_action( $hook, create_function( '', 'wp_enqueue_script( \'jquery-ui\' );' ) );
        if ( wp_is_mobile() ) {
            add_action( $hook, create_function( '', 'wp_enqueue_script( \'jquery-mobile\' );' ) );
        }
        add_action( $hook, create_function( '', 'wp_enqueue_script( \'jquery-easing\' );' ) );
        add_action( $hook, create_function( '', 'wp_enqueue_script( \'jquery-booklet\' );' ) );
